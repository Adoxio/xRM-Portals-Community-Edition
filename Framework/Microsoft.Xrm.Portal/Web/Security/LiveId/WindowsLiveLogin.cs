...
namespace Microsoft.Xrm.Portal.Web.Security.LiveId
{
	...
	/// </summary>
	internal sealed class WindowsLiveLogin
	{
		/// <summary>
		...
		public WindowsLiveLogin(bool loadAppSettings)
		{
			...
			var connectionStringName = LiveIdMembershipProvider.Current.LiveIdConnectionStringName;
			var parameters = connectionStringName.ToDictionaryFromConnectionStringName();

			string forceDelAuthNonProvisioned = parameters.FirstNotNullOrEmpty("Force Del Auth Non Provisioned", "ForceDelAuthNonProvisioned");
			...
			AppId = LiveIdMembershipProvider.Current.AppId;

			Secret = parameters.FirstNotNullOrEmpty("Secret", "Secret Key", "SecretKey");
			OldSecret = parameters.FirstNotNullOrEmpty("Old Secret", "OldSecret");
			OldSecretExpiry = parameters.FirstNotNullOrEmpty("Old Secret Expiry", "OldSecretExpiry");
			SecurityAlgorithm = parameters.FirstNotNullOrEmpty("Security Algorithm", "SecurityAlgorithm");
			PolicyUrl = parameters.FirstNotNullOrEmpty("Policy Url", "PolicyUrl");
			ReturnUrl = parameters.FirstNotNullOrEmpty("Return Url", "ReturnUrl");
			BaseUrl = parameters.FirstNotNullOrEmpty("Base Url", "BaseUrl");
			SecureUrl = parameters.FirstNotNullOrEmpty("Secure Url", "SecureUrl");
			ConsentUrl = parameters.FirstNotNullOrEmpty("Consent Url", "ConsentUrl");
			SignedOutUrl = parameters.FirstNotNullOrEmpty("Signed Out Url", "SignedOutUrl");
			bool autoRegister;
			AutoRegister = bool.TryParse(parameters.FirstNotNullOrEmpty("Auto Register", "AutoRegister"), out autoRegister) ? autoRegister : false;
		}
		...
		public WindowsLiveLogin(string settingsFile)
		{
			...
			SignedOutUrl = settings["signouturl"];
			bool autoRegister;
			AutoRegister = bool.TryParse(settings["autoregister"], out autoRegister) ? autoRegister : false;
		}
		...
		public string AppId
		{
			set
			{
				if (string.IsNullOrEmpty(value))
				{
					...
					throw new ArgumentNullException("value", "AppId: Application ID cannot be null or the empty string.");
				}
		...
		string _signedOutUrl;

		/// <summary>
		/// Sets or gets the URL to redirect to after the user has signed out. You
		/// should not have to use or change this directly.
		/// </summary>
		public string SignedOutUrl
		{
			set { _signedOutUrl = value; }

			get
			{
				if (string.IsNullOrEmpty(_signedOutUrl))
				{
					return "/";
				}

				return _signedOutUrl;
			}
		}
		...
		public class User
		{
			...
			{
				public LiveIdLoginContext(string appContext)
				{
					if (string.IsNullOrEmpty(appContext)) return;

					var contextValues = appContext.Split('$');

					foreach (var value in contextValues)
					{
						if (value.StartsWith("loginpath"))
						{
							LoginDestinationPath = value.Substring(9);
						}
						else if (value.StartsWith("registrationpath"))
						{
							RegistrationDestinationPath = value.Substring(16);
						}
					}
				}

				public string LoginDestinationPath { get; set; }
