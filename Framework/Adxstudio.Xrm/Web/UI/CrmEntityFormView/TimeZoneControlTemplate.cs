...
{
	/// <summary>
	/// Template used when rendering a Whole Number field when Format equals Timezone.
	/// </summary>
	public class TimeZoneControlTemplate : IntegerControlTemplate
	{
		/// <summary>
		...
		public TimeZoneControlTemplate(CrmEntityFormViewField field, FormXmlCellMetadata metadata, string validationGroup, IDictionary<string, CellBinding> bindings) : base(field, metadata, validationGroup, bindings)
		{
		}

		protected override void InstantiateControlIn(Control container)
		{
			var dropDown = new CrmTimeZone { ID = ControlID, CssClass = string.Join(" ", CssClass, Metadata.CssClass), ToolTip = Metadata.ToolTip, LanguageCode = Metadata.LanguageCode };
