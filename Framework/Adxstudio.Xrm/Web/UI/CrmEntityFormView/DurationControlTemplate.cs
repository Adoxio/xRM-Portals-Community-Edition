...
{
	/// <summary>
	/// Template used when rendering a Whole Number field when Format equals Duration.
	/// </summary>
	public class DurationControlTemplate : IntegerControlTemplate
	{
		/// <summary>
		...
		public DurationControlTemplate(CrmEntityFormViewField field, FormXmlCellMetadata metadata, string validationGroup, IDictionary<string, CellBinding> bindings) : base(field, metadata, validationGroup, bindings)
		{
		}

		protected override void InstantiateControlIn(Control container)
		{
			var dropDown = new CrmDuration { ID = ControlID, CssClass = string.Join(" ", CssClass, Metadata.CssClass), ToolTip = Metadata.ToolTip };
